...
const getProductsAndInfo = async (key, count, callback) => {
        ...
        try {
            ...
            for (var i = 0; i < json.hits.length; i++) {
                ...
                var shoe = new Sneaker({
                    ...
                    diffPrice:(json.hits[i].searchable_traits['Retail Price'] ? json.hits[i].lowest_ask - json.hits[i].searchable_traits['Retail Price'] : null),
                    thumbnail: json.hits[i].media.imageUrl,
                    releaseDate: `${new Date(json.hits[i].release_date).getTime()}`,
                    date:json.hits[i].release_date,
...
export default getProductsAndInfo;
